...
import edit from "../assests/pen-solid-full.svg"
        ...
        <li className="flex sm:flex-row flex-col  justify-between items-start sm:items-center p-6 bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] gap-4">
            ...
            <div className="w-full sm:w-auto">
                <div className="flex items-center gap-3">
                    <span onClick={(e) => e.stopPropagation()} className="text-xs font-semibold text-slate-400 tracking-winder">#{item.id}</span>
                    {isEdit ?
                        <>
                        ...
                        <p className={`text-lg cursor-pointer font-medium ${item.undo ? 'text-slate-300 line-through' : 'text-slate-700'}`}>{item.title}</p>}
                </div>
            </div>
            {/* right side */}
            <div className="flex flex-row sm:flex-col items-center sm:items-emd justify-between sm:justify-start w-full sm:w-auto gap-3">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={() => setIsEdit(true)} className="p-2 bg-green-500 hover:bg-slate-100 rounded-lg border border-slate-700 shadow-sm  transition group" title="Edit Task">
